...
class Derivative(Scene):
    def construct(self):
        ...
        text2 = Tex("Derivation of the derivative:")
        text3 = MathTex(r"f'(x) = \lim_{h \to 0} \frac{f(x+h) - f(x)}{h}").shift(DOWN*5/4)
        dot1 = Dot([-3.8, 0, 0], radius= 0)
        ...
        # def derivative
        line1 = Line(LEFT, RIGHT).scale(4)
        text4 = Text("The derivative is defined as the\nslope of a function at a single point", font_size= 25).shift(UP*32/9).shift(UP*3).align_to(dot1, LEFT)
        self.play(
            Write(text4),
            FadeIn(line1),
            run_time= 1
        ...
        # draw graph
        ax = Axes(y_range= [-3, 3], x_range= [-3, 3], x_length= 6, y_length= 6, tips= False).shift(DOWN*32/9)
        ...
        # add text
        text6 = Text("To approximate the slope at point A,\nyou can a pick a point B that is close to A,\nand calculate the slope inbetween A & B", font_size= 25).shift(UP*32/9).shift(UP*2).align_to(dot1, LEFT)
        self.play(Write(text6), run_time= 1)
        self.wait(3)

        # add point B
        point2 = ax.c2p(1/2 , -7/8) 
        dot3 = Dot(point2)
        text7 = always_redraw(lambda: MathTex(r"B").next_to(dot3, DR, buff= 0.1))
        self.play(
            FadeIn(dot3),
            Write(text7),
            run_time= 1
        ...
        # approximate slope
        text8 = MathTex(r"f'(x_A) \approx \frac{y_B - y_A}{x_B - x_A}").shift(UP*32/9).shift(UP*.9).align_to(dot1, LEFT)
        self.play(Write(text8), run_time= 1)

        # generalise
        text9 = Text("We can rewrite this by describing point A\nrelative to point B", font_size= 25).shift(UP*32/9).shift(DOWN*.2).align_to(dot1, LEFT)
        text10 = MathTex(r"f'(x_A) \approx \frac{f(x_A + h) - f(x_A)}{x_A + h - x_A}").shift(UP*32/9).shift(DOWN*1.1).align_to(dot1, LEFT)
        text11 = MathTex(r"f'(x_A) \approx \frac{f(x_A + h) - f(x_A)}{h}").shift(UP*32/9).shift(DOWN*1.1).align_to(dot1, LEFT)
        text12 = MathTex(r"\text{Where: }h = x_B - x_A").shift(UP*32/9).shift(DOWN*2).align_to(dot1, LEFT)
        ...
        self.play(
            FadeOut(text4),
            FadeOut(text6),
            textgroup2.animate().shift(UP*2.5),
        )
        self.wait(1)
        text14 = Text("Because of this we define the derivative\nas this approximation as h approaches 0,\nbut never reaches it:", font_size= 25).shift(UP*32/9).shift(DOWN*1.6).align_to(dot1, LEFT)
        self.play(Write(text14), run_time= 1)
        text3.move_to(ORIGIN).shift(UP*32/9).shift(DOWN*2.7).align_to(dot1, LEFT)
        ...
        # outro
        text22 = Tex("Thanks for watching!")
